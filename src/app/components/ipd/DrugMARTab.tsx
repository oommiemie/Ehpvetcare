...
import {
  Pill, Plus, X, Check, AlertTriangle, Lock, Ban, Clock,
} from "lucide-react";
...
export function DrugMARTab({ admitId, petAllergies }: { admitId: number; petAllergies?: string }) {
  const { drugs, mar, discontinueDrug, administerMAR } = useIPD();
  const { showSnackbar } = useSnackbar();
  const confirm = useConfirm();
  const [tab, setTab] = useState<"orders" | "mar">("orders");
  const [showAdd, setShowAdd] = useState(false);

  /* Wrap administerMAR with Peak-End feedback */
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <div className="bg-white rounded-full p-1 flex items-center gap-0.5 border border-gray-100" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
          {(["orders", "mar"] as const).map(t => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className="relative px-4 py-1.5 rounded-full text-[12px]"
              style={{
                color: tab === t ? "#ffffff" : "#6b7280",
                fontWeight: tab === t ? 700 : 600,
                background: tab === t ? "linear-gradient(135deg, #19a589, #0d7c66)" : "transparent",
                boxShadow: tab === t ? "0 3px 10px rgba(25,165,137,0.40)" : "none",
        ...
        <button onClick={() => setShowAdd(true)} className="ml-auto vet-btn vet-btn-primary inline-flex items-center gap-1">
          <Plus className="w-3.5 h-3.5" /> สั่งยาใหม่
        </button>
      </div>
        ...
        <section className="bg-white rounded-2xl border border-gray-100 overflow-hidden" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04), 0 8px 24px rgba(0,0,0,0.04)" }}>
          <div className="px-4 py-3 flex items-center gap-3 border-b border-gray-100/80">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-gray-900" style={{ fontWeight: 700, fontSize: 14 }}>รายการยา</h3>
              <p className="text-[11px] text-gray-500">ใช้งาน {activeDrugs.length} · ยกเลิก {patientDrugs.length - activeDrugs.length}</p>
            </div>
          </div>
          <div className="p-4">
            {patientDrugs.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-10 text-gray-400">
                ...
                <div className="text-[12px]" style={{ fontWeight: 600 }}>ยังไม่มีการสั่งยา</div>
              </div>
            ) : (
              <div className="space-y-2">
                {patientDrugs.map((d, idx) => (
                  <div key={d.id} className="relative p-3 rounded-2xl border" style={{ borderColor: d.active ? "#e5e7eb" : "rgba(239,68,68,0.20)", background: d.active ? "rgba(249,250,251,0.5)" : "rgba(239,68,68,0.03)", opacity: d.active ? 1 : 0.7 }}>
                    <div className="flex items-start gap-3">
                      <span className="inline-flex items-center justify-center w-8 h-8 rounded-full text-[11px] text-white flex-shrink-0" style={{ background: d.isControlled ? "linear-gradient(135deg, #f87171, #dc2626)" : "linear-gradient(135deg, #38bdf8, #0284c7)", fontWeight: 800 }}>
                        #{idx + 1}
                      </span>
                      <div className="flex-1 min-w-0">
                      ...
                      {d.active && (
                        <button onClick={() => askDiscontinue(d)} className="text-[11px] px-2.5 py-1 rounded-full text-rose-600 hover:bg-rose-50 inline-flex items-center gap-1" style={{ fontWeight: 600 }}>
      ...
      {tab === "mar" && (
        <section className="bg-white rounded-2xl border border-gray-100 overflow-hidden" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04), 0 8px 24px rgba(0,0,0,0.04)" }}>
          <div className="px-4 py-3 flex items-center gap-3 border-b border-gray-100/80">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-gray-900" style={{ fontWeight: 700, fontSize: 14 }}>Medication Administration Record (MAR)</h3>
              <p className="text-[11px] text-gray-500">ตารางการให้ยา — กดติ๊กเมื่อให้ยา</p>
            </div>
          </div>
          <div className="p-4">
            {patientMAR.length === 0 ? (
